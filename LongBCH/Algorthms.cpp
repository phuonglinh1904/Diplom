...
vector<Polynomial> equal_degree_factorization_by_Shoup(const Polynomial &poly, int degree, const mpz_class &q) {
    ...
    while (U.size() < poly.get_degree() / degree) {
        ...
        for (size_t l = 0; l < separatingSet.size(); ++l) {
            ...
            Polynomial x_z(vector<mpz_class>{z});
...
vector<pair<Polynomial, mpz_class>> find_root_by_Mignotte(const Polynomial& f, const mpz_class& q, int &time) {
    ...
    for (size_t i = 0; i < pi.size() - 1; ++i) {
        ...
        // cout << "h_" << i+1 << "(x) = " << hi << endl;
        hs.push_back(hi);
    }

    // Bước 2: Khởi tạo F với (f, 0)
    ...
    for (int i = pi.size() - 1; i >= 0; i--) {
        ...
        for (mpz_class j = 0; j < pi[i]; ++j) {
            ...
            for (const auto& pair : F) {
                ...
                // Computer g_j = gcd(hi-1 rem g - ξ^(e + jχ)/π_i, g)
...
vector<Polynomial> find_root_by_hybird(const Polynomial &poly, const mpz_class &q, int &time) {
    chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
    ...
    if (poly_with_root.get_degree()) {
        try {
            polys = equal_degree_factorization_by_hybird(poly_with_root, 1, q);
        } catch (const std::exception &e) {
            cerr << "Exception occurred: " << e.what() << endl;
            polys.clear();
        }
...
vector<Polynomial> solve_roots_equation_degree234(const Polynomial &poly, const mpz_class& q) {
    ...
    for (int i = pi.size() - 1; i >= 0; i--) {
        ...
        for (mpz_class j = 0; j < pi[i]; ++j) {
            ...
            for (const auto& pair : F) {
                ...
                if (gj.get_degree() >= 2 && gj.get_degree() <= 3) {
                    // call ben-or
                    auto results = solve_roots_equation_degree234(gj, q);
                    for (auto &res : results) {
                        G.push_back({res, v});
                    }
                }
                else if (gj.get_degree() == 1 ||  gj.get_degree() >= 4) {
                    auto gj_ = gj.normalize();
                    G.push_back({gj_, v});
                }
